...
`F:\DSP\SST_PRJ\4_STATCOM_CURRENT_LOOP`的SCI-A Modbus RTU通信与保护联调。
...
故障清除脉冲，仍存在的硬件故障不会被掩盖。协议V0x0105保持可写
Vdc软件过压阈值；CPLD本地连续3个ADC样本越限才锁存，硬件过压链路保持独立。
温度保护在CPLD本地以100 ms原始脉冲计数和固定阈值判断，不开放阈值下发；
当前`5000 count/100 ms`及频率方向仍需实物标定。所有阈值配置均不会解除
四路桥臂和`pwm_hold_o`的硬封锁。
